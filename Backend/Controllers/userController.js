...
const loginUser = async (req, res) => {
  try {
    ...
    // 🔴 IMPORTANT: role added in token
    const token = jwt.sign(
      {
        id: user._id,
        role: user.role, // admin / user / doctor
      },
      process.env.JWT_SECRET,
      { expiresIn: "1d" }
    );
